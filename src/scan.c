...
extern void js_output_result(const char *symbolName, const char *data,
                             const int *polygon, const unsigned polysize);
...
int scan_image(uint8_t *raw, int width, int height) {
  ...
  // set the scanner density (function will have nonzero return code on error,
  ...
  for (; symbol; symbol = zbar_symbol_next(symbol)) {
    ...
    // (Parsing two-dimensional arrays from the wesm heap introduces unnecessary
    // upstream code complexity)
...
// allocate a buffer on the WebAssembly heap to accomodate the image that is to
// be scanned.
EMSCRIPTEN_KEEPALIVE
...
// this function can be used from the javascript environment to free an image
// buffer.
EMSCRIPTEN_KEEPALIVE
