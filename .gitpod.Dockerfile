...
RUN cd ${EMSDK} \
    ...
    &&  printf "export PATH=${EMSDK}/fastcomp/bin:\$PATH\n" >> ${EMSDK}/emsdk_set_env.sh \
    \
    &&  echo "## Done"
...
RUN apt-get -qq -y update && apt-get -qq install -y --no-install-recommends \
    ...
    && rm -fr ${EMSDK}/fastcomp/emscripten/tests \
    # strip out symbols from clang (extra 50MB!)
    && find ${EMSDK}/fastcomp/bin -type f -exec strip -s {} + || true \
    && find ${EMSDK}/fastcomp/fastcomp/bin -type f -exec strip -s {} + || true \
    &&  echo "## Done"
...
ENV PATH="${EMSDK}:${EMSDK}/fastcomp/bin:${EMSDK}/emscripten/sdk:${EMSDK}/llvm/clang/bin:${EMSDK}/node/current/bin:${EMSDK}/binaryen/bin:${PATH}"
